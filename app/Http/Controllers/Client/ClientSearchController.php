...
class ClientSearchController extends Controller
{
    ...
    {
        $category = ServiceCategory::all()->take(6);
        $rawdata = DB::table('tbl_svc')
            ...
            ->orderBy('rate_point', 'DESC')->orderBy('total_reviewers', 'DESC')->paginate(8);
        return view('client.search.index', ['data' => $rawdata, 'category' => $category]);
    }
    ...
    public function search(Request $request)
    {
        ...
        $rawdata = DB::table('tbl_svc')
            ...
            ->join('tbl_servicer', 'tbl_svc.id_servicer', '=', 'tbl_servicer.id_servicer')
            ->join('tbl_svc_category', 'tbl_svc.id_svc_category', '=', 'tbl_svc_category.id_svc_category')
            ->where('tbl_svc.svc_name', 'LIKE', "%{$request->search}%")
            ->orWhere('tbl_svc_category.svc_category_name', 'LIKE', "%{$request->search}%")
            ->orWhere('tbl_svc.description', 'LIKE', "%{$request->search}%")
            ->orWhere('tbl_servicer.first_name', 'LIKE', "%{$request->search}%")
            ->orWhere('tbl_servicer.last_name', 'LIKE', "%{$request->search}%")
            ->orderBy('rate_point', 'DESC')->orderBy('total_reviewers', 'DESC')->paginate(8);
        return view('client.search.index', ['data' => $rawdata, 'category' => $category]);
    }
    public function searchCategory($category)
    {
        $categorys = ServiceCategory::all()->take(6);
        $rawdata = DB::table('tbl_svc')
            ->select(
                'tbl_svc.id_svc',
                'tbl_svc.thumbnail_image',
            ...
            ->join('tbl_servicer', 'tbl_svc.id_servicer', '=', 'tbl_servicer.id_servicer')
            ->join('tbl_svc_category', 'tbl_svc.id_svc_category', '=', 'tbl_svc_category.id_svc_category')
            ->where('tbl_svc_category.svc_category_name', '=', $category)->paginate(8);
        return view('client.search.index', ['data' => $rawdata, 'category' => $categorys]);
